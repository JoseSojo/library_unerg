...
use App\Form\A\User\Security\Method\Phone\UpdateType;
use App\Form\A\User\Security\Method\Phone\ValidateType;
 ...
 */
class PhoneProcessor extends BaseProcessor
{
    /**
    ...
    public function renderFormUpdate()
    {
        ...
        $titleWidget->setTitle($this->trans("title.phone.update"));
        $this->dynamicBuilderManager->addWidget($titleWidget);

        $subTitleWidget = new SubTitleWidget();
        $subTitleWidget->setSubTitle($this->trans("title.phone.update.help"));
    ...
    {
        $entity = $this->getUser();
        $this->dispatch(\App\AppEvents::APP_USER_PHONE_UPDATE_PRE_SUCCESS,$this->newGenericEvent($entity));
        $this->userManager->updateUser($entity);
        $this->dispatch(\App\AppEvents::APP_USER_PHONE_UPDATE_POST_SUCCESS,$this->newGenericEvent($entity));
        // Response
    ...
    public function renderFormValidate()
    {
        $entity = $this->getUser();
        $titleWidget = new TitleWidget();
        $titleWidget->setTitle($this->trans("title.phone.validate"));
        $this->dynamicBuilderManager->addWidget($titleWidget);

        $subTitleWidget = new SubTitleWidget();
        $subTitleWidget->setSubTitle($this->trans("title.phone.validate.help",["%phone%" => $entity->getPhone()]));
        $this->dynamicBuilderManager->addWidget($subTitleWidget);

        $this->dispatch(\App\AppEvents::APP_USER_PHONE_VALIDATE_PRE_SUCCESS,$this->newGenericEvent($entity));
    }

    public function persistFormValidate()
    {
        $entity = $this->getUser();
        $this->dispatch(\App\AppEvents::APP_USER_PHONE_VALIDATE_POST_SUCCESS,$this->newGenericEvent($entity));
        $this->userManager->updateUser($entity);
        // Response
        $jsonResponse = $this->newJsonResponse();
